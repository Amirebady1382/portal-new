import { pgTable, text, integer, serial, boolean, timestamp, unique, varchar, bigint, type AnyPgColumn } from "drizzle-orm/pg-core";
...
export const companies = pgTable("companies", {
  ...
  teamInfo: text("team_info"), // JSON string
  productInfo: text("product_info"), // JSON string
  marketInfo: text("market_info"), // JSON string
  financialInfo: text("financial_info"), // JSON string
  rasmioData: text("rasmio_data"), // JSON string
  aiAnalysisData: text("ai_analysis_data"), // JSON string
  signatories: text("signatories"), // JSON string: [{name, nationalId, position}] - حق امضاداران (حداکثر 2 نفر)
  // Financial Summary fields
  financialSummaryData: text("financial_summary_data"), // JSON string: extracted financial metrics from tax declaration
  taxDeclarationDocumentId: integer("tax_declaration_document_id").references(() => documents.id),
...
export const documents = pgTable("documents", {
  id: serial("id").primaryKey(),
  companyId: integer("company_id").notNull().references((): AnyPgColumn => companies.id, { onDelete: "cascade" }),
...
export const documentRequirements = pgTable("document_requirements", {
  ...
  isActive: boolean("is_active").notNull().default(true),
  // NEW: نوع دسترسی فرم: all (تمام شرکت‌ها) یا specific (شرکت‌های خاص)
...
export const contractTemplates = pgTable("contract_templates", {
  ...
  variables: text("variables"), // JSON string - متغیرهای قابل جایگزینی
  isActive: boolean("is_active").notNull().default(true),
...
export const contractFormData = pgTable("contract_form_data", {
  ...
  formData: text("form_data").notNull(), // JSON string - all form fields
...
export const services = pgTable("services", {
  ...
  requirements: text("requirements"), // الزامات و شرایط (JSON)
  isActive: boolean("is_active").notNull().default(true),
...
export const serviceRequests = pgTable("service_requests", {
  ...
  requestData: text("request_data"), // داده‌های اضافی درخواست (JSON)
...
export const contractVariables = pgTable("contract_variables", {
  ...
  validationRules: text("validation_rules"), // JSON string - قوانین اعتبارسنجی
...
// Placeholder for EmployeeBaleSettings (for backward compatibility)
export type EmployeeBaleSettings = {
  id: number;
  employeeId: number;
...
export type InsertEmployeeBaleSettings = Omit<EmployeeBaleSettings, 'id' | 'createdAt' | 'updatedAt'>;
