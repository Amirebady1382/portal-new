...
export function getVariableSource(variableName: string): VariableSource {
  ...
  // اگر نام متغیر شامل کلمات خاصی است که معمولا در فرم‌ها هستند
  if (/_amount|_date|_name|_position|_id|_number/.test(lowerName)) {
    return 'form';
  }

  // پیش‌فرض: اگر هیچ الگویی مطابقت نداشت، به عنوان ورودی دستی کارشناس در نظر گرفته شود
  // تا در پنل خلاصه نمایش داده شود و کاربر متوجه شود باید آن را پر کند
  return 'manual';
}
...
export function isVariableRequired(variableName: string, source: VariableSource): boolean {
  // متغیرهای رسمیو، محاسباتی و سیستم معمولا خودکار پر می‌شوند
  // اما اگر مقدار نداشته باشند، باید اجباری باشند تا کاربر دستی وارد کند
  if (source === 'rasmio' || source === 'calculated' || source === 'system') {
    return false;
  }

  const lowerName = variableName.toLowerCase();
  
  // تمام متغیرهای دستی و فرم به صورت پیش‌فرض اجباری هستند مگر خلافش ثابت شود
  // یا با الگوهای اجباری مطابقت داشته باشند
  if (VARIABLE_PATTERNS.required.some(pattern => pattern.test(lowerName))) {
    return true;
  }

  // برای متغیرهای ناشناخته (manual)، آن‌ها را اجباری در نظر می‌گیریم تا قرارداد خالی نماند
  if (source === 'manual') {
    return true;
  }
