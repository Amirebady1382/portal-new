...
export default function AIAnalysisWithHistory() {
  ...
  const { data: companies = [], isLoading: companiesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error('خطا در دریافت لیست شرکت‌ها');
  ...
  const { data: companyServices = [], isLoading: servicesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        ...
        throw new Error('خطا در دریافت خدمات شرکت');
      }
  ...
  const { data: chatSessions = [], isLoading: sessionsLoading, refetch: refetchSessions } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error('خطا در دریافت تاریخچه چت‌ها');
      const data = await response.json();
  ...
  const startNewAnalysis = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/companies/${selectedCompanyId}/ai-chat`, {
        method: 'POST',
        headers: {
  ...
  const loadChatSession = async (sessionId: number) => {
    try {
      ...
      if (!response.ok) throw new Error('خطا در بارگیری جلسه چت');
  ...
  const deleteSession = async (sessionId: number) => {
    try {
      ...
      if (!response.ok) throw new Error('خطا در حذف جلسه');
  ...
  const updateSessionTitle = async (sessionId: number, newTitle: string) => {
    try {
      ...
      if (!response.ok) throw new Error('خطا در به‌روزرسانی عنوان');
  ...
  const sendMessage = async (deepAnalysis: boolean = false) => {
    ...
    try {
      const response = await fetch(`/api/companies/${activeChatSession.session.companyId}/ai-chat`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        ...
        body: JSON.stringify({
          ...
          conversationHistory: activeChatSession.messages.map(m => ({
      ...
      // Refetch sessions to update message count
      refetchSessions();
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const aiResponse = await fetch(`/api/companies/${activeChatSession.session.companyId}/ai-chat`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        ...
        body: JSON.stringify({
          message: `فایل ${file.name} آپلود شد. لطفاً آن را بررسی و تحلیل کنید.`,
          sessionId: activeChatSession.session.id,
          serviceId: selectedServiceId,
          uploadedFile: actualFilename,
          conversationHistory: [...activeChatSession.messages, userMessage].map(m => ({
            role: m.type === 'user' ? 'user' : 'assistant',
            content: m.content
          }))
        })
  ...
  const handlePitchDeckUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/companies/${selectedCompanyId}/pitch-deck-analysis`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        },
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'خطا در تحلیل پیچ دک');
      }
  ...
  const renderChatMessage = (message: ChatMessage) => (
    <div
      ...
      className={`flex gap-3 p-4 ${
        message.type === 'user' 
          ? 'bg-blue-50/50 border-r-4 border-blue-400' 
          : 'bg-gray-50/50 border-r-4 border-green-400'
      } rounded-lg`}
    >
      ...
      <div className="flex-1 space-y-2">
        <div className="flex items-center gap-2">
          <span className="font-medium text-sm">
            {message.type === 'user' ? 'شما' : 'مشاور هوشمند'}
          </span>
