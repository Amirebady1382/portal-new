...
export default function AdminTemplateManagement() {
  ...
  const loadVariables = async () => {
    try {
      ...
      const response = await fetch("/api/admin/contract-variables", {
        headers: { 
      ...
      if (response.ok) {
        ...
        console.error('❌ Variables API error:', errorData);
        throw new Error(errorData.message || 'خطا در دریافت متغیرها');
      }
    } catch (error) {
  ...
  const loadTemplates = async () => {
    try {
      ...
      const contractResponse = await fetch("/api/contract-templates", {
        headers: { 
          "Authorization": `Bearer ${localStorage.getItem("auth_token")}`,
          "Content-Type": "application/json"
        }
      ...
      const reportResponse = await fetch("/api/investment-reports/templates", {
        headers: { 
          "Authorization": `Bearer ${localStorage.getItem("auth_token")}`,
          "Content-Type": "application/json"
        }
  ...
  const loadBaleMappings = async () => {
    try {
      ...
      const response = await fetch("/api/admin/bale-employee-mappings", {
        headers: { 
          "Authorization": `Bearer ${localStorage.getItem("auth_token")}`,
          "Content-Type": "application/json"
        }
      ...
      if (response.ok) {
        ...
        console.error('❌ Bale mappings API error:', response.status);
      }
    } catch (error) {
  ...
  const loadEmployees = async () => {
    try {
      ...
      const response = await fetch("/api/admin/employees-dropdown", {
        headers: { 
          "Authorization": `Bearer ${localStorage.getItem("auth_token")}`,
          "Content-Type": "application/json"
        }
  ...
  const loadTemplateMappings = async (templateId: number) => {
    try {
      const response = await fetch(`/api/admin/contract-templates/${templateId}/variables`, {
  ...
  const saveVariable = async () => {
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify(variableFormData),
      });
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        toast({ title: errorData.message || "خطا در ذخیره متغیر", variant: "destructive" });
      }
    } catch (error) {
      console.error("Error saving variable:", error);
      toast({ title: "خطا در ذخیره متغیر", variant: "destructive" });
    }
  };

  const deleteVariable = async (id: number) => {
    try {
      const response = await fetch(`/api/admin/contract-variables/${id}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${localStorage.getItem("auth_token")}` }
      });
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        toast({ title: errorData.message || "خطا در حذف متغیر", variant: "destructive" });
      }
    } catch (error) {
      console.error("Error deleting variable:", error);
      toast({ title: "خطا در حذف متغیر", variant: "destructive" });
    }
  };
  ...
  const saveBaleMapping = async () => {
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify(baleFormData),
      });
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        toast({ title: errorData.message || "خطا در ذخیره تنظیمات بله", variant: "destructive" });
      }
    } catch (error) {
      console.error("Error saving Bale mapping:", error);
      toast({ title: "خطا در ذخیره تنظیمات بله", variant: "destructive" });
    }
  };

  const deleteBaleMapping = async (id: number) => {
    try {
      const response = await fetch(`/api/admin/bale-employee-mappings/${id}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${localStorage.getItem("auth_token")}` }
      });
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        toast({ title: errorData.message || "خطا در حذف تنظیمات بله", variant: "destructive" });
      }
    } catch (error) {
      console.error("Error deleting Bale mapping:", error);
      toast({ title: "خطا در حذف تنظیمات بله", variant: "destructive" });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex">
        <Sidebar />
        <main className="flex-1 md:mr-64 p-6 mt-16">
