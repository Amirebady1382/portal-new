...
function AssociateCompanyForm() {
  ...
  const associateMutation = useMutation({
    mutationFn: async (nationalId: string) => {
      const response = await fetch("/api/companies/associate", {
        method: "POST",
...
export default function CustomerProfile() {
  ...
  const fetchRasmioData = async () => {
    ...
    try {
      const response = await fetch(`/api/rasmio/company/${companyForm.nationalId}`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error('خطا در دریافت اطلاعات از رسمیو');
      }
      ...
      if (data.basicInfo) {
        ...
        toast({
          title: "موفق",
          description: "اطلاعات شرکت از رسمیو دریافت شد",
        });
      } else {
  ...
  const addBoardMember = async () => {
    ...
    try {
      const response = await fetch(`/api/rasmio/person/${newMemberNationalId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        }
      });
