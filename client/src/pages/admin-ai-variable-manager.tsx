...
لطفاً با دقت بالا متغیرها را شناسایی کنید و فقط بخش‌هایی که واقعاً برای هر قرارداد متفاوت هستند را به متغیر تبدیل کنید.
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/ai/analyze-contract-template', {
        method: 'POST',
        headers: {
  ...
  const applyVariables = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/ai/apply-variables', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        },
  ...
  const saveAsTemplate = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/contract-templates/save-with-variables', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          categories: analysisResult.suggestedCategories
        })
      });
