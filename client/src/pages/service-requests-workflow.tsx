...
export default function ServiceRequestsWorkflow() {
  ...
  const requestsByStage = {
    ...
    completed: requests.filter((r: any) => r.workflowStage === "completed")
  };
...
function RequestCard({ 
  ...
  const canTransfer = userDepartment === "investment" && request.workflowStage === "investment_review";
  const canComplete = userDepartment === "administrative" && request.workflowStage === "administrative_review";
      ...
      <CardHeader>
        <div className="flex items-start justify-between">
          ...
          <Badge className={stageConfig?.color || "bg-gray-100"}>
            ...
            {stageConfig?.label || request.workflowStage}
          </Badge>
        </div>
      </CardHeader>
