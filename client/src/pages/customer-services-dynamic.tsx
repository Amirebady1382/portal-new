...
export default function CustomerServicesDynamic() {
  ...
  const { data: servicesData, isLoading, error } = useQuery<ServicesResponse>({
    ...
    queryFn: async (): Promise<ServicesResponse> => {
      ...
      const result = await apiRequest("GET", `/api/companies/${companyId}/services?isActive=true`);
            ...
            <div className="space-y-6">
              ...
              {workflowStatus && (
                <ServiceRequestStatus
                  stage={workflowStatus.currentStage}
                  message={
                    workflowStatus.currentStage === "investment_forms_pending"
                      ? "لطفاً فرم‌های ارزیابی را تکمیل کنید"
                      : workflowStatus.currentStage === "investment_review"
                      ? "درخواست شما در حال بررسی در واحد سرمایه‌گذاری است"
                      : workflowStatus.currentStage === "administrative_forms_pending"
                      ? "لطفاً فرم‌های واحد اداری را تکمیل کنید"
                      : workflowStatus.currentStage === "administrative_review"
               ...
               (workflowStatus.currentStage === "investment_forms_pending" || 
                ...
                <ServicesBasedFormsFixed
                  ...
                  onFormsCompleted={() => {
                    ...
                    toast({
                      title: "موفق",
                      description: "فرم‌ها با موفقیت ثبت شد",
                    });
                    // Could refetch status here
                  }}
                />
              )}
            </div>
