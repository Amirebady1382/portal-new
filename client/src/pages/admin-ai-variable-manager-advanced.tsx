...
- اولویت با استفاده از داده‌های موجود در سیستم
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      if (inputMode === 'text') {
        ...
        response = await fetch('/api/ai/analyze-text-for-variables', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
        ...
        response = await fetch('/api/ai/analyze-excel-for-variables', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
          },
        ...
        response = await fetch('/api/ai/analyze-contract-template-advanced', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
          },
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/ai/preview-variables', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        },
  ...
  const applyVariables = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/ai/apply-variables-final', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        },
  ...
  const saveAsTemplate = async () => {
    ...
    try {
      ...
      const applyResponse = await fetch('/api/ai/apply-variables-final', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        },
      ...
      const saveResponse = await fetch('/api/contract/templates', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        },
        body: saveTemplateFormData
      });
  ...
  const syncVariablesToSystem = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/ai/sync-variables-to-system', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
          'Content-Type': 'application/json'
      ...
      if (!response.ok) {
        throw new Error('خطا در سینک متغیرها');
      }
  ...
  const createSuggestedForm = async (form: FormSuggestion, department: string, serviceIds: number[] = []) => {
    setLoading(true);
    try {
      const response = await fetch('/api/ai/create-form-from-variables', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
          'Content-Type': 'application/json'
      ...
      if (!response.ok) {
        throw new Error('خطا در ایجاد فرم');
      }
                          ...
                          <CardTitle className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    const response = await fetch('/api/ai/generate-word-from-text', {
                                      method: 'POST',
                                      headers: {
                                        'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
                                        'Content-Type': 'application/json'
                                      ...
                                      body: JSON.stringify({
                                        text: inputText,
                                        variables: analysisResult.detectedVariables
                                      })
                                    });

                                    if (!response.ok) {
                                      throw new Error('خطا در تولید فایل Word');
                                    }
