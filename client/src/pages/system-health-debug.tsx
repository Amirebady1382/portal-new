...
export default function SystemHealthDebug() {
  ...
  const testHealthAPI = async () => {
    try {
      ...
      const basicHealthResponse = await fetch('/api/health');
      console.log("Basic health status:", basicHealthResponse.status);
      
      if (!basicHealthResponse.ok) {
        throw new Error(`Basic health check failed: ${basicHealthResponse.status}`);
      }
      
      const basicHealthData = await basicHealthResponse.json();
      ...
      const detailedHealthResponse = await fetch('/api/health/detailed', {
        headers: token ? {
      ...
      let detailedHealthData;
      try {
        detailedHealthData = await detailedHealthResponse.json();
        console.log("Detailed health data:", detailedHealthData);
      } catch (e) {
        detailedHealthData = { error: "Could not parse JSON" };
      }
      
      setHealthData({
        basic: basicHealthData,
        detailed: detailedHealthData,
        detailedStatus: detailedHealthResponse.status,
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg">
                          <p className="text-sm mb-2">Status: {healthData.detailedStatus}</p>
                    ...
                    onClick={async () => {
                      try {
                        ...
                        toast({
                          ...
                          description: response.ok ? `کاربر: ${data.fullName} (${data.role})` : data.message,
