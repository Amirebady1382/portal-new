...
export default function AdminInvestmentReportGenerator() {
  ...
  const fetchTemplates = async () => {
    try {
      const response = await fetch('/api/investment-reports/templates', {
        headers: {
  ...
  const fetchAIAnalysis = async () => {
    ...
    try {
      const response = await fetch(`/api/companies/${selectedCompany.id}/ai-analysis`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const generateReport = async () => {
    ...
    try {
      const response = await fetch('/api/investment-reports/generate', {
        method: 'POST',
        headers: {
