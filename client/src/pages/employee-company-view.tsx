...
export default function EmployeeCompanyView() {
  ...
  const { data: companyServicesData } = useQuery({
    ...
    queryFn: async () => {
      if (!companyId) return { services: [] };
      const response = await fetch(`/api/companies/${companyId}/services?isActive=true`, {
  ...
  const { data: allServiceForms = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/document-requirements?department=${user.department}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('auth_token')}` }
      });
      return response.json();
    },
  ...
  const { data: rasmioData, isLoading: rasmioLoading, refetch: refetchRasmio } = useQuery<any>({
    ...
    staleTime: 0, // Force fresh data on refetch
  ...
  const { 
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        throw new Error('خطا در تحلیل هوش مصنوعی');
      }
      
      const data = await response.json();
      console.log('📊 AI Analysis Data received:', data?.fromCache ? 'FROM CACHE' : 'FRESH');
      return data;
    },
    enabled: false, // Only fetch when explicitly requested
    staleTime: 1000 * 60 * 2, // Cache for 2 minutes only
    retry: 1,
  ...
  const { refetch: checkCachedAnalysis } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (data && data.teamAnalysis && data.overallRecommendation) {
        console.log('✅ Valid cached analysis found, updating main query');
        queryClient.setQueryData([`/api/companies/${companyId}/ai-analysis`], data);
        return data;
      } else {
        console.log('⚠️ Invalid cached analysis, ignoring');
        return null;
      }
    },
  ...
  const refetchAIWithRefresh = async (forceRefresh = false) => {
    console.log(`🔄 ${forceRefresh ? 'Force refresh' : 'Smart fetch'} AI analysis...`);
    ...
    if (forceRefresh) {
      ...
      try {
        ...
        console.log('✅ Fresh analysis received:', freshData);
      ...
      // Smart fetch - just trigger the existing query
  ...
  const deleteCompanyMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || 'خطا در حذف شرکت');
      }
    ...
    onError: (error: Error) => {
      alert(error.message || "خطا در حذف شرکت");
    }
  });
  ...
  const deleteFormSubmissionMutation = useMutation({
    mutationFn: async (submissionId: number) => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || 'خطا در حذف فرم');
      }
    ...
    onError: (error: Error) => {
      alert(error.message || "خطا در حذف فرم");
    }
  });
          ...
          <TabsContent value="reports" className="space-y-6">
              ...
              <TabsContent value="ai-analysis" className="space-y-6">
                <Card>
                  ...
                  <CardContent>
                    {(aiLoading || isManuallyRefreshing) ? (
                      <div className="flex items-center justify-center h-32">
                        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mr-4"></div>
                        <span>{isManuallyRefreshing ? 'در حال تولید تحلیل جدید...' : 'در حال بارگیری...'}</span>
                      </div>
                    ) : (
                      <div className="space-y-6">
