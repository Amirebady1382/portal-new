  ...
  const generateContract = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.downloadUrl) {
          ...
          if (downloadResponse.ok) {
            ...
            try {
              ...
              a.href = url;
              a.download = result.fileName || `contract_${selectedCompany.name}_${Date.now()}.docx`;
              ...
              toast({
                title: "موفق",
                description: `قرارداد با موفقیت تولید شد: ${result.fileName || 'قرارداد'}`
              });
            } catch (blobError) {
        ...
        throw new Error(errorData.message || 'خطا در تولید قرارداد');
      }
    } catch (error) {
    ...
    try {
      const response = await fetch('/api/contracts/generate', {
        ...
        body: JSON.stringify({
          ...
          companyId: selectedCompany.id,
          contractNumber: generateContractNumber(),
          customFields: guaranteeFormData,
          variables: guaranteeFormData
        })
      });
