...
export default function TwoFactorLogin() {
  ...
  const handleStep1Submit = async (data: Step1FormData) => {
    setIsLoading(true);
    try {
      const result: any = await apiRequest("POST", '/api/auth/login', data);
  ...
  const handleStep2Submit = async (data: Step2FormData) => {
    ...
    setIsLoading(true);
    try {
      const result: any = await apiRequest("POST", '/api/auth/login/verify', { 
        userId: loginData.userId, 
        code: data.otp 
      });

      if (result.token && result.step === 2) {
        // ذخیره token
  ...
  const handleResendOTP = async () => {
    ...
    setIsLoading(true);
    try {
      const result: any = await apiRequest("POST", '/api/auth/login/resend', loginData);
