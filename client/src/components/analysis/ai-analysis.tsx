...
export default function AIAnalysis({ companyId }: AIAnalysisProps) {
  ...
  const { data: aiAnalysis, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest("GET", `/api/companies/${companyId}/ai-analysis`);
  ...
  const handleRegenerateAnalysis = async () => {
    ...
    try {
      ...
      const response = await apiRequest("GET", `/api/companies/${companyId}/ai-analysis?forceRefresh=true`);
