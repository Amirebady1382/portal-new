...
export default function ServicesBasedForms({ department, companyId }: ServicesBasedFormsProps) {
  ...
  const { data: services = [] } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/services?department=${department}&isActive=true`, {
        headers: {
  ...
  const { data: serviceRequirements = [] } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/document-requirements?department=${department}&companyId=${companyId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        }
      });
  ...
  const { data: serviceRequestsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/service-requests?department=${department}&companyId=${companyId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        }
      });
