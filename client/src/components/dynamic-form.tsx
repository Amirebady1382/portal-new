...
export default function DynamicForm({ requirement, companyId, onSuccess }: DynamicFormProps) {
  ...
  const uploadFileMutation = useMutation({
    mutationFn: async ({ file, fieldName }: { file: File, fieldName: string }) => {
      ...
      const response = await apiRequest("POST", `/api/companies/${companyId}/documents`, formData);
      
      // apiRequest already throws on non-2xx statuses and parses JSON
      return response;
    },
    onMutate: ({ fieldName }) => {
            ...
            <div className="space-y-2">
              {!uploadedFile && (
                <Input
                  ...
                  className={error ? 'border-red-500' : ''}
                  accept={Array.isArray(field.validation?.allowedFormats) ? field.validation.allowedFormats.map(f => `.${f}`).join(',') : undefined}
                  disabled={isUploading}
                />
              )}
              
              {Array.isArray(field.validation?.allowedFormats) && (
