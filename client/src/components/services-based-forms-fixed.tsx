...
export default function ServicesBasedFormsFixed({ 
  ...
  // Fetch form submissions with useQuery for reactivity
  const { data: submissionsData, refetch: refetchSubmissions } = useQuery({
    queryKey: ["/api/form-submissions", { companyId }],
    queryFn: async () => {
      if (!companyId) return [];
      const data = await apiRequest<any[]>("GET", `/api/form-submissions?companyId=${companyId}`);
      return data || [];
    },
    enabled: !!companyId
  });

  // Keep formSubmissions state in sync with query data
  useEffect(() => {
    if (submissionsData) {
        ...
        <CardContent>
          <div className="space-y-3">
            <DocumentUpload 
              ...
              onUploadComplete={() => {
                refetchSubmissions(); // Use refetch instead of fetchData
              }}
            />
            ...
            <CardContent>
              ...
              {requirements.length > 0 ? (
                <div>
                  ...
                  {requirements.length > 1 ? (
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="mt-4">
                        {renderRequirementCard(requirements[currentStep], service.id)}
                      </div>
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                if (serviceRequestId) {
                                  try {
                                    setLoading(true);
                                    await apiRequest("POST", `/api/service-requests/${serviceRequestId}/mark-forms-completed`, {
                                      stage: department
                                    });
                                    toast({
                                      title: "تبریک",
                                    ...
                                    toast({
                                      title: "خطا",
                                      description: error.message || "خطا در ثبت نهایی فرم‌ها",
                                      variant: "destructive"
                                    });
                                  } finally {
                                    setLoading(false);
                              ...
                              disabled={incompleteRequiredForms.length > 0 || loading}
                            >
                              تکمیل نهایی
                          ...
                          onClick={async () => {
                            ...
                            if (serviceRequestId) {
                              try {
                                ...
                                toast({
                                  title: "تبریک",
                                  description: "فرم‌ها تایید و به مرحله بعد ارسال شد.",
                                });
                                if (onFormsCompleted) onFormsCompleted();
                              } catch (error: any) {
                                toast({
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>هیچ فرم یا مدرکی برای این مرحله نیاز نیست.</p>
                  <p className="text-sm mt-2 mb-6">می‌توانید مستقیماً درخواست را به مرحله بعد ارسال کنید.</p>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          toast({
                            title: "موفق",
                            description: "مرحله بدون نیاز به فرم تکمیل شد.",
                          });
                          if (onFormsCompleted) onFormsCompleted();
                        } catch (error: any) {
                          toast({
                            title: "خطا",
                            description: error.message || "خطا در ارجاع",
                            variant: "destructive"
                          });
                        } finally {
                          setLoading(false);
