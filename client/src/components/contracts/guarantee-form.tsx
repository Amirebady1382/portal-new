...
export default function GuaranteeForm({ companyId, templateId, onSubmit, initialData }: GuaranteeFormProps) {
  ...
  const loadSavedData = async () => {
    try {
      const response = await apiRequest("GET", `/api/contract-form-data/${companyId}/${templateId}`);
  ...
  const loadRasmioData = async () => {
    ...
    try {
      const response = await apiRequest("POST", `/api/companies/${companyId}/rasmio`);
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      const result: any = await apiRequest("POST", '/api/contract-form-data', {
        companyId,
        templateId,
        formType: 'guarantee',
        formData
