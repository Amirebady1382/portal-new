...
export class DatabaseStorage implements IStorage {
  ...
  async getDocument(id: number): Promise<Document | undefined> {
    ...
    return result.rows[0] as unknown as Document | undefined;
  }
  ...
  async createDocumentRequirement(requirementData: Omit<DocumentRequirement, 'id' | 'createdAt' | 'updatedAt'> & { companyIds?: number[] }): Promise<DocumentRequirement> {
    const now = new Date().toISOString();
    const result = await db.execute(
      `INSERT INTO document_requirements (title, department, category, description, fields, is_required, "order", is_active, access_type, service_id, created_by, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING *`,
      [
        ...
        (requirementData as any).serviceId || null,
  ...
  async getMappingsByForm(requirementId: number) {
    try {
      ...
      logger.error('Error fetching mappings by form', 'storage', {
        requirementId,
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
