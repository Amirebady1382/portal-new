...
// Helper to safely parse JSON or return the object if already parsed
...
const handleAIChat = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // If no session provided, create new session
    if (!currentSessionId) {
      const newSession = await storage.createAIChatSession({
        userId: req.user.userId,
        companyId,
        title: isInitial 
          ? `تحلیل ${company.name} - ${new Date().toLocaleDateString('fa-IR')}`
          : `چت با ${company.name} - ${new Date().toLocaleDateString('fa-IR')}`,
        serviceId: serviceId
      });
    ...
    try {
      ...
      const response = await Promise.race([
        ...
        new Promise<any>((_, reject) => 
          setTimeout(() => reject(new Error('Anthropic API request timed out after 15s')), 15000)
        )
      ]);
      ...
      if (i === PREMIUM_MODELS.length - 1) {
        ...
        console.log(`🔄 Attempting to use GapGPT as a fallback...`);
        try {
          // Convert the Anthropic message format to what GapGPT expects
          let systemContext = '';
          const userMessages = [];
          ...
          const gapGptResponse = await gapGPTService.generateFromMessages(userMessages, systemContext);
          console.log(`✅ SUCCESS with GapGPT fallback`);
          return gapGptResponse;
        } catch (gapGptError: any) {
          console.error(`🚨 GapGPT FALLBACK ALSO FAILED! Error:`, gapGptError.message);
          throw error; // Throw the original Anthropic error
        }
      }
...
async function generateInitialAnalysis(companyId: number, company: any, serviceId?: number): Promise<string> {
  try {
    ...
    const companyData: any = {
      ...
      companyPanels: {
        teamInfo: safeParseJson(company.teamInfo),
        productInfo: safeParseJson(company.productInfo),
        marketInfo: safeParseJson(company.marketInfo),
        financialInfo: safeParseJson(company.financialInfo),
      },
    };
    ...
    if (company.rasmioData) {
      try {
        const rasmio = safeParseJson(company.rasmioData);
        if (rasmio) {
...
توجه: این اولین پیام در مکالمه است، بنابراین کامل و جامع باشید اما طولانی نشود.`;
    ...
    const finalPrompt = `${prompt}\n\nلطفاً این تحلیل را بازنویسی کنید:\n1. با سلام و معرفی خودتان به عنوان مشاور هوشمند صندوق گیلان شروع کنید\n2. به صورت متنی طبیعی، روان و جذاب ارائه دهید (نه JSON!)\n3. همه بخش‌های مهم را پوشش دهید\n4. نمره‌ها و امتیازها را به صورت فارسی ذکر کنید\n\nتوجه: این اولین پیام در مکالمه است، بنابراین کامل و جامع باشید اما طولانی نشود.`;

    const conversationalText = await tryPremiumAnthropicCall([{ role: 'user', content: finalPrompt }], 3000, 0.4);
...
- پاسخ فارسی و روان باشد`;
    ...
    // Add current user message with context instructions
    messages.push({ 
      role: 'user', 
