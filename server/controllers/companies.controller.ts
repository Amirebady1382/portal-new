...
export class CompaniesController {
  ...
  async getCompanyAIAnalysis(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const onlyCache = req.query.onlyCache === 'true';
      const serviceId = req.query.serviceId ? parseInt(req.query.serviceId as string) : undefined;

      console.log(`🎯 Controller: AI Analysis request for company ${companyId}, forceRefresh: ${forceRefresh}, onlyCache: ${onlyCache}, serviceId: ${serviceId || 'none'}`);
      ...
      const analysisResult = await companiesService.getCompanyAIAnalysis(companyId, forceRefresh, serviceId, onlyCache);

      res.json(analysisResult);    } catch (error) {
  ...
  async getFinancialSummary(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      let data = (company as any).financialSummaryData;
      if (typeof data === 'string') {
        try {
          data = JSON.parse(data);
        } catch (e) {
          console.error("Error parsing financialSummaryData:", e);
        }
      }
