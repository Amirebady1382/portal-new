...
export class AIAnalysisService {
  ...
  async analyzeCompany(companyData: CompanyAnalysisData): Promise<AnalysisResult> {
    ...
    try {
      ...
      logger.info('شروع تحلیل با Claude AI...', 'ai-analysis');
...
پاسخ شما باید یک JSON معتبر باشد با ساختار زیر:
{
  ...
  "overallRecommendation": {
    ...
    "nextSteps": ["گام ۱", "گام ۲"]
        ...
        logger.info('تحلیل Claude دریافت شد', 'ai-analysis');
      } catch (claudeError) {
        logger.warn('⚠️ خطا در فراخوانی Claude AI، تلاش برای استفاده از GapGPT به عنوان جایگزین...', 'ai-analysis', { error: claudeError });

        try {
          analysisText = await gapGPTService.generateResponse(analysisPrompt, `${systemPrompt}
          
  پاسخ شما باید یک JSON معتبر باشد با ساختار زیر:
  {
    "companyOverview": "معرفی کوتاه شرکت",
    "teamAnalysis": { "score": 5, "strengths": [], "weaknesses": [], "summary": "" },
    "productAnalysis": { "score": 5, "marketPotential": "", "competitiveAdvantage": "", "summary": "" },
    "marketAnalysis": { "score": 5, "marketSize": "", "competition": "", "trends": "", "summary": "" },
    "financialAnalysis": { "score": 5, "capitalStructure": "", "growthPotential": "", "summary": "" },
    "riskAnalysis": { "score": 5, "mainRisks": [], "mitigationStrategies": [], "summary": "" },
    "formAnalysis": { "score": 5, "completedForms": [], "keyInsights": [], "documentsQuality": "", "missingInfo": [], "summary": "" },
    "overallRecommendation": { "score": 5, "recommendation": "neutral", "reasoning": "", "nextSteps": [] }
  }`);
          logger.info('تحلیل GapGPT با موفقیت دریافت شد', 'ai-analysis');
        } catch (gapError) {
          logger.error('❌ خطا در فراخوانی GapGPT نیز رخ داد', 'ai-analysis', gapError instanceof Error ? gapError : new Error(String(gapError)));
          throw claudeError; // Throw the original error or handle accordingly
        }
      }
