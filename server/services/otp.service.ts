  ...
  async sendOTP(phone: string, purpose: 'login' | 'register' | 'reset'): Promise<OTPSendResult> {
    try {
      ...
      try {
        ...
        await db.execute(
          `DELETE FROM otp_codes WHERE phone = ? AND purpose = ? AND is_used = 0`,
          [phone, purpose]
        );
        ...
        // Insert new OTP
        await db.execute(
          `INSERT INTO otp_codes (phone, code, purpose, attempts, is_used, expires_at, created_at) VALUES (?, ?, ?, 0, 0, ?, datetime('now'))`,
