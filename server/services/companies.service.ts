...
export class CompaniesService {
  ...
  async enrichCompanyData(companyId: number): Promise<any> {
    ...
    return await rasmioService.enrichCompanyData(company.nationalId);
  }
   ...
   */
  async getCompanyAIAnalysis(companyId: number, forceRefresh: boolean = false, serviceId?: number): Promise<any> {
    console.log(`🎯 AI Analysis Request: Company ${companyId} - ${forceRefresh ? 'FORCE REFRESH' : 'CACHE FIRST'}`);
    ...
    if (!forceRefresh && (company as any).aiAnalysisData) {
      try {
        ...
        if (cachedAnalysis && cachedAnalysis.teamAnalysis && cachedAnalysis.overallRecommendation) {
          ...
          console.log(`📅 Cached analysis timestamp: ${cachedAnalysis.analysisTimestamp || 'unknown'}`);
          return {
            ...cachedAnalysis,
            fromCache: true,
          ...
          console.log(`⚠️ Cached analysis structure incomplete, regenerating...`);
        }
      } catch (error) {
        console.log(`⚠️ Error parsing cached analysis, regenerating:`, error);
      }
    }
