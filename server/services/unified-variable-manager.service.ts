...
export class UnifiedVariableManager {
  ...
  /**
   * تعمیر متغیرهای شکسته با استراتژی Gap Removal
   * این روش تگ‌های XML میانی را حذف می‌کند بدون اینکه کل ساختار را تخریب کند
   */
  private fixBrokenVariables(xmlContent: string): { fixedContent: string; fixCount: number } {
    let content = xmlContent;
    let totalFixCount = 0;

    // 1. حذف تگ‌های غلط‌گیر (Proofing) که عامل اصلی شکستن متغیرها هستند
    const proofPatterns = [/<w:proofErr[^>]*\/>/g, /<w:noProof[^>]*\/>/g, /<w:lastRenderedPageBreak[^>]*\/>/g];
    proofPatterns.forEach(p => {
      content = content.replace(p, '');
    });

    // 2. ادغام براکت‌های شکسته { <tags> { -> {{
    // ما فاصله بین براکت‌ها را با حذف مرزهای تگ w:t ادغام می‌کنیم
    const braceGapPattern = /\{(?:\s*<[^>]*>\s*)+\{/g;
    content = content.replace(braceGapPattern, (match) => {
      totalFixCount++;
      return match.replace(/<\/w:t>.*?<w:t[^>]*>/g, '');
    });

    const closeBraceGapPattern = /\}(?:\s*<[^>]*>\s*)+\}/g;
    content = content.replace(closeBraceGapPattern, (match) => {
      totalFixCount++;
      return match.replace(/<\/w:t>.*?<w:t[^>]*>/g, '');
    });

    // 3. ادغام محتوای داخلی متغیرها {{ <tags> var <tags> }}
    // این الگو هر چیزی بین {{ و }} که شامل تگ است را پیدا می‌کند
    const internalGapPattern = /\{\{(?:[^{}]*?<[^>]*>[^{}]*?)+?\}\}/g;
    content = content.replace(internalGapPattern, (match) => {
      // فقط حذف مرزهای تگ w:t داخلی برای contiguous کردن متن
      const healed = match.replace(/<\/w:t>.*?<w:t[^>]*>/g, '');
      
      // اگر هنوز تگی باقی مانده (مثلا w:rPr)، آن را کاملا پاکسازی می‌کنیم چون وسط متغیر مجاز نیست
      if (healed.includes('<') || healed.includes('>')) {
        const textOnly = healed.replace(/<[^>]*>/g, '').replace(/\s+/g, '').trim();
        if (textOnly.startsWith('{{') && textOnly.endsWith('}}')) {
          totalFixCount++;
          // برای امنیت، در یک تگ w:t تمیز برمی‌گردانیم اگر از قبل در match بود
          if (match.includes('<w:t')) {
             return `<w:t>${textOnly}</w:t>`;
          }
          return textOnly;
        }
      }
      
      if (healed !== match) totalFixCount++;
      return healed;
    });

    // 4. استانداردسازی نهایی و حذف براکت‌های اضافی احتمالی
    content = content.replace(/\{{3,}/g, '{{').replace(/\}{3,}/g, '}}');

    if (totalFixCount > 0) {
      console.log(`🛠️ Gap removal applied. Healed ${totalFixCount} fragments.`);
    }
   ...
   */
  private generateValidation(_name: string, type: VariableType): ValidationRules {
   ...
   */
  private _findOldestCacheKey(): string | null {
