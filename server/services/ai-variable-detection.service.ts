...
- UNIQUE بودن original را تضمین کنید - هیچ دو متغیری نباید original یکسانی داشته باشند`;
  ...
  private async analyzeWithClaude(prompt: string, model: string, retryCount: number = 0): Promise<AIAnalysisResult> {
    ...
    const MAX_RETRIES = 3; // 🆕 افزایش به 3 برای شانس بیشتر AI

    try {
      // Use latest Claude models - prioritize Claude 4
      let modelId = 'claude-sonnet-4-20250514'; // 🚀 Default to Claude 4 - most powerful
...
${prompt}`;
      ...
      try {
        const response = await anthropicClient.messages.create({
          model: modelId,
          max_tokens: 8192, // افزایش به 8K برای اطمینان از کامل شدن JSON
          temperature: 0, // 🆕 کاهش به 0 برای دقت بیشتر و تکرار کمتر
          messages: [
            {
              role: 'user',
        ...
        console.warn('⚠️ Anthropic API failed in variable detection, trying GapGPT fallback...', claudeError);
        try {
          // GapGPT might not support separate system message in the same way, but we can combine them.
          // Since the prompt already contains the system context (from analyzeWithAI calling buildAnalysisPrompt),
          // we just pass enhancedPrompt as input.
          contentText = await gapGPTService.generateResponse(enhancedPrompt);
          console.log('✅ GapGPT response received for variable detection');
        } catch (gapError) {
          console.error('❌ GapGPT fallback for variable detection also failed:', gapError);
          throw claudeError; // Rethrow original error if fallback fails
        }
      }
      ...
      // If first pattern fails, try more specific patterns
      if (!jsonMatch) {
      ...
      try {
        ...
        try {
          ...
          console.log('🔍 Attempting to extract partial valid data from cleaned JSON...');
          ...
          try {
            ...
            if (partialResult && partialResult.detectedVariables && partialResult.detectedVariables.length > 0) {
              console.log(`✅ Extracted ${partialResult.detectedVariables.length} variables from cleaned JSON`);
              result = partialResult;
            }
          } catch (partialError) {
            console.log('❌ Partial extraction from cleaned JSON failed, trying with raw JSON...');
            try {
              partialResult = this.extractPartialJSON(jsonMatch[0]);
              if (partialResult && partialResult.detectedVariables && partialResult.detectedVariables.length > 0) {
                console.log(`✅ Extracted ${partialResult.detectedVariables.length} variables from raw JSON`);
                result = partialResult;
              }
            } catch (rawError) {
              console.log('❌ Partial extraction from raw JSON also failed');
            }
          }
          ...
          if (!result) {
            ...
            console.log('🚨 JSON parsing completely failed after all retries');
            throw new Error(`JSON parsing failed after ${MAX_RETRIES} retries: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
          }
        }
      }
      ...
      if (!validation.valid) {
        console.error('❌ JSON structure validation failed:', validation.errors);

        // Retry if we haven't exhausted attempts
        if (retryCount < MAX_RETRIES) {
          console.log(`🔄 Retrying due to invalid structure... (${retryCount + 1}/${MAX_RETRIES})`);

          // Add validation errors to next prompt so AI knows what to fix
          const errorFeedback = `
...
- ❌ Using the same suggestion name for multiple different fields
- ❌ Using suggestion like "{{beneficiary_registration_office}}" for name, address, birth date, etc.
- ✅ Each field must have its OWN UNIQUE suggestion name
...
${prompt}`;
      ...
      // Don't retry on other API errors, only on parsing/connection errors
