...
export class ContractsService {
   ...
   */
  async updateContractTemplate(id: number, updates: Partial<ContractTemplate>): Promise<any> {
    return await storage.updateContractTemplate(id, updates);
  }
  ...
  async prepareContractData(
    ...
    try {
      ...
      if (formSubmissions.length > 0) {
        ...
        // 🚀 بررسی برای تمام متغیرهایی که منبع آن‌ها غیر از محاسباتی و سیستم است
        // چون هر متغیری (حتی دستی) ممکن است به یک فیلد فرم مپ شده باشد
        const mappableVariables = templateVariables.filter((v: any) =>
          typeof v === 'object' && v.name && v.source !== 'calculated' && v.source !== 'system'
        );

        logger.info(`🔍 Checking form mappings for ${mappableVariables.length} variables in template`, 'contracts');

        // 🚀 دریافت تمام mappings برای همه variables یکجا
        const variableIds = mappableVariables.map((v: any) => v.id).filter(Boolean);
        ...
        if (variableIds.length > 0) {
          ...
          // ایجاد Map برای دسترسی سریع به جدیدترین form submissions
          ...
          for (const s of sortedSubmissions) {
            if (!submissionMap.has(s.requirementId)) {
              submissionMap.set(s.requirementId, s);
            }
          }

          // پردازش هر variable با استفاده از mappings (priority-based fallback)
          for (const variable of mappableVariables) {
            ...
            if (mappings && mappings.length > 0) {
              for (const mapping of mappings) {
                ...
                const formData = safeJsonParseObject(submission.formData as any, {}) as Record<string, any>;
                const fieldValue = formData[mappingDetails.field_name];

                if (fieldValue !== undefined && fieldValue !== null && fieldValue !== '') {
                  mappedFormData[variable.name] = fieldValue;
                  logger.debug(`🎯 Mapped variable ${variable.name} from form field ${mappingDetails.field_name}`, 'contracts');
                  break; 
                }
              }
            }
  ...
  private async legacyProcessTemplate(templateBuffer: Buffer, replacementData: Record<string, any>): Promise<Buffer> {
    try {
      ...
      // Heal fragmented tags even in legacy mode to prevent lexer errors
      const healedBuffer = await unifiedVariableManager.fixDocumentVariables(templateBuffer);
      
      const doc = new Docxtemplater(new PizZip(healedBuffer), {
        paragraphLoop: true,
        linebreaks: true,
      });
  ...
  async deleteContractTemplate(id: number): Promise<boolean> {
    return await storage.deleteContractTemplate(id);
  }

  /**
   * Get contract for download with security checks
   */
