...
export class DocumentsService {
    ...
    const createZipStream = async (res: any): Promise<void> => {
      ...
      for (const document of documents) {
        try {
          ...
          archive.file(absolutePath, { name: docOriginalName || `document_${(document as any).id}` });
        } catch (error) {
  ...
  async getFileForDownload(documentId: number): Promise<{
    ...
    // Extract filename from stored path (to handle cross-environment issues like Docker vs Windows)
    const filename = path.basename(docFilePath);
    ...
    if (!exists) {
      logger.error("File not found at path", "documents", undefined, undefined, { filePath, originalPath: docFilePath });
      return null;
    }
