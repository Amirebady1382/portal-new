...
- Badge ها برای امتیازات`;
    ...
    try {
      ...
      htmlContent = content.type === 'text' ? content.text : '';
      tokensUsed = response.usage.input_tokens + response.usage.output_tokens;
    } catch (error) {
      console.warn('⚠️ Claude API for report generation failed, trying GapGPT fallback...', error);

      try {
        htmlContent = await gapGPTService.generateResponse(prompt, systemContext);
        tokensUsed = 0; // GapGPT might not return tokens in the same format
      } catch (gapError) {
        console.error('❌ GapGPT fallback for report generation also failed:', gapError);
        throw error;
      }
    }
